...
{{define "contents"}}
     ...
     <div class="panel panel-default">
         <div class="panel-body">
            <div id="chart-trend" class="" style="height: 230px;"></div>
         </div>
     </div>
...
{{define "javascript"}}
    ...
    <script src="/static/modules/{{.ctrl}}/trend.js"></script>
    <script src="/static/modules/{{.ctrl}}/formatter.js"></script>
    <script>
        var params = "start_date={{.filter.StartDate}}&end_date={{.filter.EndDate}}&fast_paging=on&equip_id={{.filter.EquipId}}{{range .filter.EventType}}&event_type={{.}}{{end}}{{range .filter.OrgId}}&org_id={{.}}{{end}}{{range .filter.GroupId}}&group_id={{.}}{{end}}";
        // console.log(url);
    </script>
{{end}}